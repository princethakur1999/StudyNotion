...
function DrawerItems({ navDrawer, setNavDrawer }) {
    ...
    const fetchCatalogItems = async () => {
        ...
        try {
            ...
            console.log('Error while fetching category items');
    ...
    return (
        ...
        <>
            <div onClick={() => setNavDrawer(false)} className='flex justify-end text-3xl mb-6'>
...

            {
                ...
                (
                    ...
                    >
                        ...
                        />
                        <p className='text-richblack-25 text-xl font-medium tracking-wide'>
                                ...
                                item.title == 'Catalog' ?
                                    (
                                        <>
                                            ...
                                            <div
                                                className={`flex flex-col gap-1 bg-richblack-600 ${catalogClicked
                                                    ? 'h-full py-1 px-1 mx-2 my-3'
                                                    : 'h-0 overflow-hidden py-0 px-0 mx-0 my-0'
                ...
                )}
            {
                ...
                (
                    <div className='flex h-full items-end gap-4 '>
                        <Link
                            className={` customGradient-Bg text-richblack-900 font-semibold  py-2 px-2 rounded-lg border border-[#808080a1]  transition-all  flex-1 text-center`}
                            to={'/signup'}
                            onClick={() => setNavDrawer(false)}
                        >
                            Sign up
                        ...
                        <Link
                            className={` customGradient-orangeBg text-richblack-900 font-semibold  py-2 px-2 rounded-lg border border-[#808080a1]  transition-all  flex-1 text-center`}
                            to={'/login'}
                            onClick={() => setNavDrawer(false)}
                        >
                            Login
